...
```{r}
# your code
select(filter(starwars, homeworld == 'Alderaan'), name)
```
...
```{r}
# your code
select(filter(starwars, sex == 'male' & (hair_color == 'brown' | hair_color == 'black')), name, height, mass)
```
...
```{r}
# your code
select(filter(starwars, sex == 'female'), name, homeworld)
```
...
```{r}
# your code
select(slice(arrange(starwars, desc(height)), 1:5), name, homeworld, species)
```
...
```{r}
# your code
select(slice(arrange(starwars, desc(mass)), 1:5), name, homeworld, species)
```
